...
const GameList = (props) => {
    ...
    const mappedGames = sortedGames.map((game, key) =>  <li 
                                                    key={key}>
                                                    <div onClick={() => onGameClick(game.game_id)}>
                                                        <div>{ game.game_id }_</div>
                                                        <div>{ game.teamA } ( <span className="bold"> { game.scoreA }</span> ) vs { game.teamB } ( <span className="bold">{ game.scoreB }</span> )</div>
                                                    </div>
                                                    <img src={trashCanIcon} alt='trashcan' onClick={() => onCloseClick(game.game_id)} />
                                                </li> ); 
